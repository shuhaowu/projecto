...
(function() {
  ...
  var create_todo_controller_tests = function(archived) {

    // Turn off jquery effects, which means animations returns instantly
    jQuery.fx.off = true;
    var baseUrl = window.API_PREFIX + "/projects/" + projectKey + "/todos/";
    var archivedstate = archived ? "1" : "0";
    var namemod = archived ? "Archived" : "";

    // All we need is to have archived_todos in the path
    var $location = {
      path: function() {
        return "http://projecto/app#/projects/projectkey/" + (archived ? "archived_todos" : "todos");
      }
    };

    describe(namemod + "TodoItemController", function() {
      ...
      it("should figure out archived state", function() {
        expect(scope.is_archived).toBe(archived);
      });

      if (!archived) {
        // Editing todos only for non-archived
        ...
        // Archiving only for non-archived
      ...
      it("should toggle todo", function() {
        ...
        var todo = new TodoItem(todoKey, project, undefined, archived);
      ...
      it("should mark todos as done", function() {
        ...
        scope.markDone(todo);
        $httpBackend.expectPOST(baseUrl + todoKey + "/markdone?archived=" + archivedstate, {done: true}).respond({status: "okay"});
        $httpBackend.flush();
        expect(todo.data.done).toBe(true);

        scope.markDone(todo);
        $httpBackend.expectPOST(baseUrl + todoKey + "/markdone?archived=" + archivedstate, {done: false}).respond({status: "okay"});
        $httpBackend.flush();
        expect(todo.data.done).toBe(false);
      });

      it("should delete todos", function() {
        var todo = new TodoItem(todoKey, project, {title: "Test"}, archived);
        spyOn(scope, "$emit");

        scope.deleteTodo(todo, 0);
        $httpBackend.expectDELETE(baseUrl + todoKey + "?archived=" + archivedstate + "&really=1").respond({status: "okay"});
        $httpBackend.flush();
    ...
    describe(namemod + "TodosController", function() {
      ...
      beforeEach(angular.mock.inject(function($rootScope, $controller, _$httpBackend_, Todos) {
        ...
        scope.currentProject = project;
        scope.todolist = new TodoList(project, {archived: archived});
      ...
      // NON-ARCHIVED TESTS
      if (!archived) {
        // New todo box doesn't appear on screen, either
        ...
        // Cannot create a new archived item.
      ...
      it("should update", function() {
        ...
        if (!archived) {
          $httpBackend.expectGET(baseUrl + "tags/?archived=" + archivedstate).respond({
            tags: ["tag1", "tag2"]
          });
        }
        ...
        var urlpostfix = archived ? "?archived=1" : "filter?page=1&showdone=0&shownotdone=1&tags=tag1&tags=tag2&tags=+";
        $httpBackend.expectGET(baseUrl + urlpostfix).respond({
          todos: list,
          currentPage: 1,
          totalTodos: list.length,
    ...
    describe(namemod + "SingleTodoController", function() {
      var $httpBackend, scope, controller, newcontroller, TodoItem, tododiv;
      ...
      it("should update and initialize", function() {
        scope.update();
        $httpBackend.expectGET(baseUrl + todoKey + "?archived=" + archivedstate).respond(returnedtododata);
        $httpBackend.flush();
  ...
  create_todo_controller_tests(false);
  create_todo_controller_tests(true);
})();
